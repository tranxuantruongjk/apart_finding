...
const Management = ({ manageRoute }) => {
  ...
  useEffect(() => {
    ...
    const linkFind = Object.values(linksList).find(link => link.attributes.href.value === pathname);
    linkFind.classList.add("active");
    const linksFilter = Object.values(linksList).filter((link) => link !== linkFind);
